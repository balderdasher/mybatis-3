...
public class PooledDataSource implements DataSource {
  ...
  /**
   * {@link PoolState} 对象,记录池化的状态
   */
  private final PoolState state = new PoolState(this);

  /**
   * {@link UnpooledDataSource} 对象
   */
  ...
  /**
   * 强制关闭连接池中的所有活跃连接和空闲连接
   * Closes all active and idle connections in the pool.
   */
  public void forceCloseAll() {
    synchronized (state) {
      expectedConnectionTypeCode = assembleConnectionTypeCode(dataSource.getUrl(), dataSource.getUsername(), dataSource.getPassword());
      // 按连接激活时间倒序逐个关闭池中的活跃连接,后激活的连接先被关闭,此举是为了保证回滚事务的先后顺序
      ...
      // 按连接的激活时间倒序关闭池中的空闲连接,后激活的连接先被关闭,此举是为了保证回滚事务的先后顺序
    ...
    synchronized (state) {
      ...
      if (conn.isValid()) {
        ...
        if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
          ...
          // 使用被移除的连接属性构造新的连接对象并放入空闲连接集合中
          PooledConnection newConn = new PooledConnection(conn.getRealConnection(), this);
          state.idleConnections.add(newConn);
          newConn.setCreatedTimestamp(conn.getCreatedTimestamp());
          newConn.setLastUsedTimestamp(conn.getLastUsedTimestamp());
          // 设置原连接实现,避免还有使用方在使用它,万一再次调用则抛异常
          conn.invalidate();
          if (log.isDebugEnabled()) {
   ...
   * @return 连接代理对象 {@link PooledConnection}
   * @throws SQLException
   */
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        // 池中没有空闲连接?
          ...
          // 当前连接数已经大于最大活动连接数
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) {
              ...
              // 使用之前获取到的首个激活并且已超时的连接属性创建新的 PooledConnection 连接对象
        ...
        if (conn != null) {
          ...
          // 通过 ping 检查连接是否有效
          if (conn.isValid()) {
            // 非自动提交时进行回滚,虽然之前已经进行过一次回滚操作,但是这里的连接不是之前那个,而是根据之前被移除的连接新建出来的
            if (!conn.getRealConnection().getAutoCommit()) {
              conn.getRealConnection().rollback();
            }
            // 设置连接的属性
  ...
  protected boolean pingConnection(PooledConnection conn) {
    ...
    try {
      // 判断真实的连接是否已经关闭,若已经关闭,则ping操作肯定失败
